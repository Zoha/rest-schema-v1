...
module.exports = async (fields, check, type) => {
    const validations = [];
    for (let key in fields) {
        let field = fields[key];
        let validation = check(key);
        const isGetRequest = type === "count" || type === "index";
        ...
        // if type of check is query (index and count)
        ...
        if ((field.sanitizer || field.isBranched) && !isGetRequest) {
            ...
            validation = validation.customSanitizer(
                applyCustomSanitizers(field)
            );
        }
