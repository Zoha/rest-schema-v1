...
app.use(
  ...
  resource({
    ...
    fields: {
      ...
      something2: {
        type: String,
        get: (value, { record, route }) => {
          if (route === "index") return record.name;
        },
        set: (value, { record, route }) => {
          if (route === "index") return record.name;
        }
      }
    },
  ...
  it("returns json for index route", async () => {
    await request(app)
      ...
      .expect(res => {
        ...
        expect(res.headers).to.haveOwnProperty("x-hook-global-before-response");
        expect(res.headers).to.haveOwnProperty("x-hook-index-before");
        expect(res.headers).to.haveOwnProperty("x-hook-index-before-response");
      });
  });
