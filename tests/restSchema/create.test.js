...
describe("create route of schema resource", () => {
  ...
  it("will validate type of branched array value for 2 level up", async () => {
    ...
    await request(app)
      ...
      .expect(res => {
        ...
        expect(response.array2[0].array[0]).to.haveOwnProperty("notCreatable");
        expect(response.array2[0].array[0].notCreatable).to.be.equal("ok");
        expect(response.array2[0].array[0]).to.not.haveOwnProperty("other");
      });
    const records = await Model.find({});
    const record = records[0].toObject();
    ...
    expect(record.array2[0].array[0]).to.not.haveOwnProperty("notCreatable");
  });
  ...
  it("will response with 400 for empty body", async () => {
    await request(app)
      .post("/default3")
      .send({
        name: "name"
